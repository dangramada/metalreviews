...
| 9 | Puppeteer launched without `--no-sandbox` flag | Low (reliability) | Yes | Primarily a reliability concern on Render's container environment, not a security issue. Silent failure mode: scraper runs but Metal Storm results are dropped |
