...
Maintain all brainstorm, plan, review or finish output files under a subfolder named after the current task (e.g. artifacts/superpowers/task_name_here/

## 6) Token Efficiency & Context Mastery

To minimize context bloat and prevent "context amnesia":

- **Prioritize Partial Reads:** Use `view_file` with line ranges (`StartLine`, `EndLine`) whenever possible.
- **Rationale:** Reading a 2,000-line file consumes ~20,000 tokens—roughly double the entire system's base overhead (~10k tokens).
- **Structural Mapping First:** When opening a file for the first time, especially if large (>500 lines), I MUST read the first 50 lines (imports/class headers) or `grep` the file structure for ~300 tokens to build a "mental map" before diving into a specific snippet.
- **Bridge Reading:** If logic spans multiple non-contiguous areas, use multiple small `view_file` ranges rather than one giant block.
- **Strategy:** Always use `grep_search` or `find_by_name` first to locate the relevant code block before viewing it.a
