...
- `criteria-calibration-adaptive-degree-escalation.md` — design checkpoint (not implemented): why a fixed-window plateau-detection rule was rejected (punctuated accuracy gains), why coverage-based (`computeTouchCounts`/`.min`/`.max`) is the right signal instead, open questions for the next session; also notes an unfixed LP-infeasibility bug found while diagnosing this
