...
- `criteria-calibration-dantzig-fix` — **active, ready to merge.** Switches `simplex.ts`'s
  pivoting from Bland's rule to Dantzig's (both phases) and adds a post-solve feasibility
  guard + near-singular-pivot detection. Fixes the LP crash that stalled Dan's session at
  question #59, and a live silent-failure that had been persisting all-zero preference
  weights. Rollback tag `pre-dantzig-fix`. Details:
