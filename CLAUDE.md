...
- `criteria-calibration-1000minds-comparative-research.md` — 2026-08-16: read-only comparative research running the same criteria/levels model on 1000minds' PAPRIKA calibration to contrast against Slant Take's punctuated accuracy growth; documents 1000minds' degree-2-completion interstitial and per-candidate "impossible combination" delete action as worth learning from, and its silent-until-reveal accuracy display as a blunt (not superior) alternative to Slant Take's already-proposed confidence/coverage split; research only, feeds a future Concept Draft
