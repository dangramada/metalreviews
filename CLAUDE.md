...
`album-rating-soft-gate` merged to `master` on 2026-08-09 via merge commit `e3d5d64`
(rollback tag `pre-merge-album-rating-soft-gate` on the prior tip) — branch retained per
convention, not deleted. Removes the hard calibration-tier block on album rating
(`useCalibrationGate`/`FavoritesPage.tsx`'s `handleRate`) — an explicit reversal of the
30 July gating decision, prompted by Dan hitting the block himself right after the same
day's gate-redesign migration correctly re-graded his account below Medium. Rating is
now always reachable; a dismissible (non-blocking) nudge dialog still offers calibration
first (with a Cancel option), and a low-confidence warning badge (red-on-black "!",
flush square beside the rank badge, shown only for tier `'none'`) plus a plain "Score
confidence: {tier}" label on `AlbumRatingPage` surface the existing Low/Medium/High/
Very-High tiers — no new accuracy math. `tsc`/lint/vitest clean (224/224 tests) both
pre- and post-merge. Full detail: `docs/decisions/album-rating-soft-gate.md`.
