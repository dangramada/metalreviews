...
// ---------------------------------------------------------------------------------------
// Solver-crash regression fixture (2026-08-16) — the 44-answer log that makes the LP solver
// break down numerically. SYNTHETIC, not real user data: generated by driving synthetic
// oracle #8 ("noisy", 12% random-flip rate, uniform ground truth) through the real
// nextAction/CalibrationSession/computeCommitState loop until it crashed, i.e. this is the
// exact answer log the production elicitation driver itself produced. Full provenance:
// docs/decisions/criteria-calibration/criteria-calibration-synthetic-oracles.md (oracle #8,
// crashed at round 44) and the impact assessment that reproduced it end to end,
// criteria-calibration-near-singular-pivot-impact.md.
//
// At n=44 (degree 3, 6x5 shape) solveValues throws from computeChebyshevCenter with
// post-solve-infeasible / minPivotMagnitude ~1.9e-9. At n=43 it solves fine, which is what
// makes it usable for testing the trim-the-last-answer recovery path.
//
// This fixture is expected to KEEP THROWING until the EPS = 1e-9 ratio-test root-cause fix
// lands (deferred-work.md item 3) — solverCrashFixture.test.ts asserts exactly that, so if
// that fix ever makes this log solve, the test fails loudly rather than the safety-net tests
// silently passing against an input that no longer exercises anything.
// ---------------------------------------------------------------------------------------

export const SOLVER_CRASH_LEVELS_PER_CRITERION = [5, 5, 5, 5, 5, 5];

/** The degree the driver had escalated to when the crash hit — needed to resume the session
 *  at the same point the page would. */
export const SOLVER_CRASH_DEGREE = 3;

export const SOLVER_CRASH_ANSWERS: RealSessionRound[] = [
  { profileA: { 0: 5, 1: 1 }, profileB: { 0: 1, 1: 5 }, result: 'equal' },
  { profileA: { 0: 5, 2: 1 }, profileB: { 0: 1, 2: 5 }, result: 'equal' },
  { profileA: { 0: 5, 3: 1 }, profileB: { 0: 1, 3: 5 }, result: 'equal' },
  { profileA: { 0: 5, 4: 1 }, profileB: { 0: 1, 4: 5 }, result: 'equal' },
  { profileA: { 0: 5, 5: 1 }, profileB: { 0: 1, 5: 5 }, result: 'A' },
  { profileA: { 1: 5, 2: 1 }, profileB: { 1: 1, 2: 5 }, result: 'equal' },
  { profileA: { 1: 5, 3: 1 }, profileB: { 1: 1, 3: 5 }, result: 'A' },
  { profileA: { 1: 5, 4: 1 }, profileB: { 1: 1, 4: 5 }, result: 'equal' },
  { profileA: { 1: 5, 5: 1 }, profileB: { 1: 1, 5: 5 }, result: 'equal' },
  { profileA: { 2: 5, 3: 1 }, profileB: { 2: 1, 3: 5 }, result: 'equal' },
  { profileA: { 2: 5, 4: 1 }, profileB: { 2: 1, 4: 5 }, result: 'equal' },
  { profileA: { 2: 5, 5: 1 }, profileB: { 2: 1, 5: 5 }, result: 'equal' },
  { profileA: { 3: 5, 4: 1 }, profileB: { 3: 1, 4: 5 }, result: 'A' },
  { profileA: { 3: 5, 5: 1 }, profileB: { 3: 1, 5: 5 }, result: 'equal' },
  { profileA: { 4: 5, 5: 1 }, profileB: { 4: 1, 5: 5 }, result: 'equal' },
  { profileA: { 0: 4, 1: 3 }, profileB: { 0: 3, 1: 4 }, result: 'equal' },
  { profileA: { 0: 4, 1: 2 }, profileB: { 0: 2, 1: 4 }, result: 'equal' },
  { profileA: { 3: 2, 5: 3 }, profileB: { 3: 3, 5: 2 }, result: 'equal' },
  { profileA: { 0: 3, 3: 4 }, profileB: { 0: 4, 3: 3 }, result: 'equal' },
  { profileA: { 0: 2, 3: 4 }, profileB: { 0: 4, 3: 2 }, result: 'equal' },
  { profileA: { 0: 3, 3: 3 }, profileB: { 0: 4, 3: 2 }, result: 'equal' },
  { profileA: { 0: 2, 4: 4 }, profileB: { 0: 4, 4: 2 }, result: 'equal' },
  { profileA: { 0: 3, 3: 2 }, profileB: { 0: 2, 3: 3 }, result: 'equal' },
  { profileA: { 3: 2, 4: 4 }, profileB: { 3: 4, 4: 2 }, result: 'equal' },
  { profileA: { 2: 3, 4: 4 }, profileB: { 2: 4, 4: 3 }, result: 'equal' },
  { profileA: { 2: 3, 4: 2 }, profileB: { 2: 2, 4: 3 }, result: 'equal' },
  { profileA: { 0: 3, 4: 2 }, profileB: { 0: 2, 4: 3 }, result: 'equal' },
  { profileA: { 0: 2, 5: 4 }, profileB: { 0: 4, 5: 2 }, result: 'equal' },
  { profileA: { 0: 3, 1: 2, 4: 4 }, profileB: { 0: 2, 1: 4, 4: 3 }, result: 'equal' },
  { profileA: { 0: 4, 1: 3, 2: 2 }, profileB: { 0: 3, 1: 2, 2: 4 }, result: 'equal' },
  { profileA: { 1: 1, 2: 4, 3: 2 }, profileB: { 1: 4, 2: 2, 3: 1 }, result: 'equal' },
  { profileA: { 0: 4, 2: 3, 4: 1 }, profileB: { 0: 1, 2: 4, 4: 3 }, result: 'equal' },
  { profileA: { 2: 3, 4: 5, 5: 3 }, profileB: { 2: 2, 4: 4, 5: 5 }, result: 'equal' },
  { profileA: { 1: 2, 3: 3, 5: 4 }, profileB: { 1: 1, 3: 5, 5: 3 }, result: 'equal' },
  { profileA: { 0: 3, 2: 2, 5: 3 }, profileB: { 0: 2, 2: 4, 5: 2 }, result: 'equal' },
  { profileA: { 3: 1, 4: 3, 5: 5 }, profileB: { 3: 4, 4: 2, 5: 3 }, result: 'equal' },
  { profileA: { 2: 4, 3: 1, 5: 2 }, profileB: { 2: 2, 3: 4, 5: 1 }, result: 'equal' },
  { profileA: { 3: 5, 4: 1, 5: 1 }, profileB: { 3: 1, 4: 4, 5: 2 }, result: 'equal' },
  { profileA: { 0: 2, 2: 5, 4: 1 }, profileB: { 0: 3, 2: 3, 4: 2 }, result: 'equal' },
  { profileA: { 3: 4, 4: 3, 5: 3 }, profileB: { 3: 2, 4: 4, 5: 4 }, result: 'equal' },
  { profileA: { 2: 3, 3: 3, 4: 4 }, profileB: { 2: 5, 3: 4, 4: 1 }, result: 'equal' },
  { profileA: { 1: 3, 2: 4, 4: 3 }, profileB: { 1: 4, 2: 1, 4: 5 }, result: 'equal' },
  { profileA: { 0: 5, 2: 1, 3: 1 }, profileB: { 0: 1, 2: 4, 3: 2 }, result: 'equal' },
  { profileA: { 1: 1, 2: 2, 4: 5 }, profileB: { 1: 2, 2: 3, 4: 3 }, result: 'equal' },
];
